...
def _max_clustering_cython(im, im_fgnd_mask, rad):
    sx = im.shape[1]
    sy = im.shape[0]
    r = int(rad + 0.5)
    ...
    local_max_val = np.zeros([sy, sx], dtype=float)
    local_max_ind = np.zeros([sy, sx], dtype=np.int64)
    ...
    maxpath_np = np.zeros([1000, 2], dtype=np.int64)
    maxpath = maxpath_np
    end_x, end_y, end_pos, end_ind, end_max_ind = 0, 0, 0, 0, 0
    ...
    for i in range(num_pixels):
            ...
            while not peak_found[end_y, end_x]:
                ...
                end_x = end_ind % sx
                end_y = end_ind//sx
                end_max_ind = local_max_ind[int(end_y), int(end_x)]
